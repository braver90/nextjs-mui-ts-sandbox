export default class Formatter {
  ...
  constructor(language?: string) {
    ...
    try {
      if ('getCanonicalLocales' in Intl && language) {
        const intl = Intl as any;
        const langString =
          language.indexOf(',') > 0 ? language.split(',')[0] : language;
        const langCheck = (Intl as any).getCanonicalLocales(langString);
        currentLanguage = !!langCheck ? langString : currentLanguage;
      }
    } catch (err) {
      console.error(
        "Browser language couldn't been detected. Default En is selected."
      );
    }
  ...
  toCurrency(
    ...
    if ((precision && !isNaN(precision)) || currency || showName) {
  ...
  private toCompact(num: number, opts: Intl.NumberFormatOptions) {
    const compactFormatter = Intl.NumberFormat('en', {
      ...
      maximumSignificantDigits:
        !!opts.maximumFractionDigits && opts.maximumFractionDigits > 0
